...
class Exclusivos extends MY_Front {
    ...
    public function login($idVinculo,$confirmacao,$transf) {
        if($idVinculo){
            $condicoes2 = array(
                'condicoes' => array(
            ...
            $idV = $this->superModel->select('prestador_cliente', $condicoes2);
            $this->data['vinculo'] = $idVinculo;
        }

        $this->session->set_userdata("vinculo", $idVinculo);
        $this->data['tipo'] = $idV[0]->tipo_quem_incluiu;
        $this->layout->view('loginExclusivo',$this->data);
    }

    public function logar() {
        //ver acesso e restritamodel
        $this->session->set_userdata('v', $this->input->post('id_vinculo'));
        $this->restritaModel->login();
    }


    public function cadastro($idVinculo, $tipo) {
        
        $this->session->set_userdata('v', $idVinculo);
        
        $this->data['vinculo'] = $idVinculo;
        $this->data['tipo'] = $tipo;
        if($tipo == 5){    
            $this->data['estadosTotais'] = $this->superModel->select('estado');
            insertTag('js', 'jquery.maskedinput.min.js?v=1.0', $this->data); 
            $this->layout->view('loginEmpresa',$this->data);
        }else{
            redirect('prestador');
        }
    }
