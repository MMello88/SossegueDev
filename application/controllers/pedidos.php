...
class Pedidos extends MY_Front {
    ...
    public function index($cidade = '') {
        $this->session->set_userdata('inbound_lead', 'pedidos/' . $cidade);
        
        if ($this->session->userdata('id_orcamento')){//redirecionar para a manutenção
        ...
        if($this->form_validation->run('pedidos/realizar')) {
			...
			foreach ($this->data['MenuSubCategorias'] as $item){
				if ($item->id_subcategoria == $this->input->post('subcategoria')){
					$this->session->set_userdata('url', $item->link);
					
					$cidade = $this->getCidadeById($this->input->post('id_cidade'));
					$this->session->set_userdata($cidade);
                    $this->session->set_userdata('cidade_link', $cidade->link);

					redirect('Manutencao/'. removeAcentos($item->link). '/' . $cidade->link);
				}
			}
        } else {
            $clausulas = array(
