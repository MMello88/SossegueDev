	...
	public function usuario() {
        if($this->form_validation->run('restrita/dados/atualiza')) {
            ...
            $usuario = array(
                ...
                'telefone2' => $this->input->post('celular')
            );
            ...
            $clausulas = array(
                'row' => true,
                'select' => 'usuario.id_usuario, nome, email, telefone, telefone2, id_cidade, bairro, endereco, numero, complemento, id_usuario_endereco',
                'condicoes' => array(
                    array(
                        'campo' => 'usuario.id_usuario',
                        'valor' => $id
    ...
    public function renovarAnuncio() {
        if($this->form_validation->run('restrita/anunciar')) {
            ...
            if($fatura['total'] > 0) {
                $data = array(
                    'row' => TRUE,
                    'select' => 'nome, email, telefone2, cpf',
                    'condicoes' => array(
                        array(
                            'campo' => 'id_usuario',
                            'valor' => $idUsuario
                ...
                $produto = array(
                    'comprador' => array(
                        'nome' => $usuario->nome,
                        'email' => $usuario->email,
                        'ddd' => substr(preg_replace('/\(|\)/', '', substr($usuario->telefone2, 0, 3)), 0, 2),
                        'telefone' => str_replace('-', '', substr($usuario->telefone2, 5, 10)),
                        'cpf' => $usuario->cpf
                    ),
