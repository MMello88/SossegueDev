...
class Pedidos extends MY_Restrita {
    ...
    public function lista_pedidos($id_orcamento){
        ...
        $sql = "SELECT id_orcamento, id_subcategoria
                  FROM tbl_pedido 
                 WHERE id_orcamento = $id_orcamento
                 GROUP BY id_orcamento, id_subcategoria";
        $pedido_grp = $this->superModel->query( $sql );
        foreach ($pedido_grp as $key => $p_grp) {
            $sql = "SELECT * 
                      FROM tbl_prof_pergunta_resposta_pedido
                     WHERE id_subcategoria = $p_grp->id_subcategoria
                       AND id_orcamento    = $id_orcamento";
            $profRespPedido = $this->superModel->query( $sql );
            $pedido_grp[$key]->profRespPedido = $profRespPedido;

            foreach ($pedidos as $p_key => $pedido) {
                if($pedido->Servico->categoria->id_subcategoria == $p_grp->id_subcategoria){
                    $pedido_grp[$key]->pedidos[] = $pedido;
                }
            }
        }

        //print_r($pedido_grp);
        $jsonPedidos = json_encode($pedido_grp);
        echo $jsonPedidos;
    }
