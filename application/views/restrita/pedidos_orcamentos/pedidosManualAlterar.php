...
<?php echo form_open('restrita/pedidos_orcamentos/gravar', array('id' => 'formPedido', 'data-wow-duration' => '2s', 'onSubmit' => 'return false;')); ?>

<div class="col-sm-12 form-inline ">
    PEDIDO NRO:
    <input class="form-control" style="text-align:center"type="number" placeholder="Nro do Pedido" name="nro" id="nro" value="<?php echo $nroPedido; ?>" disabled> 
</div>
<div class="col-sm-12">
    SOLICITANTE: 
    <input class="form-control" type="text" placeholder="Nome Solicitante" name="nome_solicitante" value="<?php echo ($pedidosOrcamentos->nome_sol == "" ? $user[0]->nome : $pedidosOrcamentos->nome_sol); ?>" disabled>
</div>
<?php if ($pedidosOrcamentos->nome_aprov != "") { ?>
    <div class="col-sm-12">
        APROVADOR: 
        <input class="form-control" type="text" placeholder="Nome Solicitante" name="nome_solicitante" value="<?php echo $pedidosOrcamentos->nome_aprov; ?>" disabled>
    </div>
<?php } ?>
<div class="col-sm-12">
...
<div class="col-sm-4">
    <h5><a class="btn btn-info" href="<?php echo base_url('restrita/pedido/' . ($this->session->userdata('url') ?$this->session->userdata('url'): "Eletricista")); ?>/<?php echo $idPedidoOrcamento; ?>"><font color="#333">Adicionar mais serviços</font></a></h5>
</div>
<?php if ($pedidos) { ?>
    <div class="col-sm-12"> 
        <div class="row">
                ...
                <div class="col-xs-12 col-md-12 team-box">
                    ...
                    <div class="col-xs-12 col-md-2" style="padding-top: 15px;">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="text" id="qntd<?php echo $pedido->id_pedido; ?>" name="qntd[<?php echo $pedido->id_pedido; ?>]" class="form-control"  placeholder="Qtde" style="text-align: center;" value="<?php echo $pedido->qntd; ?> " maxlength="2" required readonly="true">
                    ...
                    <div class="col-xs-12">
                        <ul class="list-inline">
                            <li><a href="<?php echo base_url('restrita/pedidos_orcamentos/actionRemoverPedido/' . $pedido->id_pedido); ?>"><font face="Arial" size="3" color="#046C00">Remover Pedido</font></a></li>
                        </ul>
                    </div>
                </div>
            <?php } ?>
        ...
        <?php if ($user[0]->id_tipo_usuario == 5 && $pedidosOrcamentos->nome_sol == "" ) { ?>
            <div class="col-sm-12">
                <select name="setor" id="setor" class="form-control" required>
                    <option value="" class="option_cad">SETOR</option>
                    <?php foreach ($setores as $setor) { ?>
                        <option value="<?php echo $setor->nome; ?>" class="option_cad" <?php echo ( $setor->nome == $pedidosOrcamentos->localidade ? "selected" : "") ?>><?php echo $setor->nome; ?></option>
                    <?php } ?>
                </select> 
            </div>
        <?php } else { ?>
            <div class="col-sm-12">
                <input class="form-control" name="setor_mostrar" placeholder="<?php echo ($pedidosOrcamentos->localidade == "" ? $setor : $pedidosOrcamentos->localidade); ?>"  value="" disabled>
                <input class="form-control" type="hidden" name="setor" id="setor"   value="<?php echo ($pedidosOrcamentos->localidade == "" ? $setor : $pedidosOrcamentos->localidade); ?>" >
            </div>
        <?php } ?>
        <div class="col-sm-12">
        ...
        <?php if ($pedidos) { ?>
            <div class="col-sm-3">
                <button type='submit' form="formPedido" value='Submit' class="btn btn-success btn-block" id="gravar">Salvar</button>
            </div>
        <?php } ?>
        <?php echo form_close(); ?>
        <?php if ($pedidosOrcamentos->nome_sol == "") { ?>
            <div class="col-sm-3">
                <a class="btn btn-success btn-block" href="<?php echo base_url('restrita/pedidos_orcamentos/cancelar/' . $pedidosOrcamentos->id_pedido_de_orcamento); ?>">Cancelar Pedido</a>
            </div>
        <?php } else { ?>
            <div class="col-sm-3">
                <a class="btn btn-success btn-block" href="<?php echo base_url('restrita/pedidos_orcamentos/cancelar_edicao'); ?>">Cancelar</a>
            </div>
        <?php } ?>
    </div>
</div>
